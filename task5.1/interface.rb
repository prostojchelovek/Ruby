class Interface
  ...
  def take_the_place
    ...
    @wagons_passenger.each { |wagon| wagon.take_place if wagon.identifier == i_wagon }
  end
  ...
  def occupy_volume
    ...
    @wagons_passenger.each { |wagon| wagon.fill_volume(places) if wagon.identifier == i_wagon }
  end
  ...
  def information_about_stations
    @stations.each do |station|
      puts station.name
      station.block_station { |train| puts "Номер поезда:#{train.number} Тип:#{train.class.name} Кол-во вагонов: #{train.wagons.size} " }
    end
  end
    ...
    @trains.each do |train|
      puts "Номер поезда #{train.number}"
        if train.is_a?(PassengerTrain)
          train.block_train { |wagon| puts "Номер вагона:#{wagon.identifier} Тип вагона:#{train.class.name} Кол-во свободных мест: #{wagon.free} Кол-во занятых мест: #{wagon.occupied}" }
        else
          train.block_train { |wagon| puts "Номер вагона:#{wagon.identifier} Тип вагона:#{train.class.name} Кол-во свободного объема: #{wagon.free} Кол-во занятого объема: #{wagon.occupied}" }
        end
    end
  end
  ...
  def unhook_wagon_by_train_number
    if ! @trains.empty?
      ...
      @trains[i_train].wagons.each { |wagon| puts "#{wagon} - #{i_wagon += 1}" }
      i_wagon = gets.chomp.to_i
      wagon = @trains[i_train].wagons[i_wagon - 1]
